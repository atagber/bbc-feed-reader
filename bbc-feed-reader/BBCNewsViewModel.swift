...
public class BBCNewsViewModel: BBCNewsViewModelType, BBCNewsViewModelInputs, BBCNewsViewModelOutputs {
  ...
  init() {
    ...
    self.categories = self.viewDidLoadProperty.signal.map { categories }
    
    self.tappedNewsCategoryProperty.signal.skipNil().observeValues { currentCategory.value = $0 }
    self.viewDidLoadProperty.signal.observeValues { currentCategory.value = defaultCategory }
    ...
    let fetchResult: Signal<Status, NoError> = needToFetchNewsPosts
      .flatMap(.merge) { AppEnvironment.current.storageService.fetchNewsPosts(forCategory: $0) }
    ...
    self.newsPosts = Signal.merge(databaseNewsPosts, clearListOnFetching)
      .map { newsPosts in newsPosts.sorted(by: { $0.0.publicationDate > $0.1.publicationDate }) }
    
    //
    
    let newsPostsAreFetching: Signal<Status, NoError> = needToFetchNewsPosts.map { _ in Status.fetching }
    
    let newsPostsListIsEmpty: Signal<Status, NoError> = self.newsPosts.map { $0.count == 0 ? Status.empty : Status.none }
    
    self.currentStatus = Signal.merge([fetchResult, newsPostsListIsEmpty, newsPostsAreFetching])
    
    // show news post
    
    self.showNewsPost = self.addButtonTappedProperty.signal.map { NewsPost(customForCategory: currentCategory.value!) }
  }
  ...
  public var inputs: BBCNewsViewModelInputs { return self }
  public var outputs: BBCNewsViewModelOutputs { return self }
}
