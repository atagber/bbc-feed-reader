...
public class NewsCategoriesViewModel: NewsCategoriesViewModelType, NewsCategoriesViewModelInputs, NewsCategoriesViewModelOutputs {
  ...
  init() {
    ...
    self.updateCategoriesList = Signal.combineLatest(self.viewDidLoadProperty.signal, self.categoriesProperty.signal)
      .map { _, categories in categories }
    
    let categoriesProperty = self.categoriesProperty
    let categoryWasTapped = self.categoryCellTappedIndexProperty.signal.skipNil().map { categoriesProperty.value[$0] }
  ...
  public var inputs: NewsCategoriesViewModelInputs { return self }
  public var outputs: NewsCategoriesViewModelOutputs { return self }
}
