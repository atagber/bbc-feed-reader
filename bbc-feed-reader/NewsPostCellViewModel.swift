...
internal class NewsPostCellViewModel: NewsPostCellViewModelType, NewsPostCellViewModelInputs,
  NewsPostCellViewModelOutputs {
  init() {
    let newsPost = self.newsPostProperty.signal.skipNil()

    self.titleLabelText = newsPost.map {
      $0.title
    }
    self.summaryLabelText = newsPost.map {
      $0.summary
    }
    ...
    self.favoriteIconHidden = newsPost.map {
      !$0.isFavorite
    }
  }
  ...
  internal var inputs: NewsPostCellViewModelInputs {
    return self
  }
  internal var outputs: NewsPostCellViewModelOutputs {
    return self
  }
}
